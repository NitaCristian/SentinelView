...
while True:
    ...
    tracked_count = 0  # Initialize the counter for tracked cars
    for i, box in enumerate(boxes):
        x, y, w, h = [int(v) for v in box]

        # Draw bounding box on the original frame
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        tracked_count += 1  # Increment the counter for each tracked car

    # Display the current count of tracked cars
    cv2.putText(frame, f'Tracked Cars: {tracked_count}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
